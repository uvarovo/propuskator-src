...
export default class AccessSubjectTokenDisable extends Base {
    ...
    async execute({ id }) {
        try {
            ...
            await sequelize.transaction(async transaction => {
                await accessSubjectToken.update({ enabled: false, updatedAt: sequelize.fn('CURRENT_TIMESTAMP', 3) }, { transaction });
                await accessSubjectToken.updateReaderTokens({ transaction });

                await Notification.create({
                    message              : 'Subject has disabled token',
                    type                 : Notification.types.USER_ACTIONS.SUBJECT_DISABLED_TOKEN,
